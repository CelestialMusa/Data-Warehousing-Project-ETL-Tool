...
namespace Energy_Cons_DW_BI
{
    public partial class MainForm : Form
    {
        ...
        {
            string monthName = "";
            #region Month name switch statement
            switch (val)
            {
                case 1:
                    monthName = "January";
        ...
        private async void storesCVSToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            openDiag.Title = "Open Stores Source File";
            openDiag.FileName = "stores";
            openDiag.Filter = "CSV Files | *stores*.csv";
        ...
        private async void storeDataCVSToolStripMenuItem_Click_1(object sender, EventArgs e)
        {
            OpenFileDialog openDiag = new OpenFileDialog();
            openDiag.Title = "Open Store Data CSV File";
            openDiag.Filter = "CSV Files | *store*data*.csv";
            openDiag.FileName = "store_data";

            if (openDiag.ShowDialog() == DialogResult.OK)
            {
                IEnumerable<string> csvLines = null;

                csvLines = File.ReadLines(openDiag.FileName);
        ...
        private async void insertRecordsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            #region Junk Code
            ...
            foreach (var item in storeList)
            {
                ...
                foreach (var dataItem in matchedList)
                {
                    try
                    {
                        ...
                        listBox1.Items.Add(listBox1.Items.Count + "\t" + dataItem.SHOP_CODE + "\t" + dataItem.SHOP_NAME + "\t\t" + dataItem.SHOP_LOCATION + "\t\t" + "Main " + dataItem.KWH_MAIN + "Bakery " + dataItem.KWH_BAKERY + "Fridge " + dataItem.KWH_REFRIG + "Other " + dataItem.KWH_OTHER);
                        listBox1.TopIndex = listBox1.Items.Count - 1;
                    }
                    catch (Exception ex)
                    {
        ...
        private void truncateTablesToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            if(MessageBox.Show("All Data will be deleted, do you wish to continue?","Confirm Data Deletion",MessageBoxButtons.YesNo,MessageBoxIcon.Question) == DialogResult.Yes)
            {
                handler = new DatabaseHandler();
                try
                {
                ...
                {
                    label1.Text = "Command Finished executing with errors!";
                }
                
            }
        }

        private void createTablesToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Tables will be dropped and re-created, do you wish to continue?", "Confirm re-creating tables", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                handler = new DatabaseHandler();
